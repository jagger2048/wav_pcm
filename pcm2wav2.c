...
typedef struct
{
	...
	unsigned short bitDepth; // wBitsPerSample
...
int main(int argc, char *argv[])
{
	...
	formatchunk.bitDepth = atoi(argv[5]);
	printf("%d %d %d\n", formatchunk.numChannels, formatchunk.sampleRate, formatchunk.bitDepth);
	formatchunk.blockAlign = formatchunk.numChannels * (formatchunk.bitDepth >> 3);// ch * bitDepth / 8
	printf("blockAlign %d\n", formatchunk.blockAlign);
	formatchunk.bytesPerSecond = formatchunk.blockAlign * formatchunk.sampleRate;
	printf("bytesPerSecond %d\n", formatchunk.bytesPerSecond);
